    ...
    def gather_project_info(self) -> dict:
        ...
        try:
            ...
            project_info["description"] = input(
                f"{Fore.GREEN}Project description: {Style.RESET_ALL}"
            )
            project_info["version"] = self._get_valid_input(
                "Enter project version [0.1.0]: ", InputValidator.validate_version
            )
            project_info["author"] = (
                input(f"{Fore.GREEN}Author name: {Style.RESET_ALL}") or ""
            )
